...
class Openletters extends CI_Controller
{
	...
	{
		$users		= $this->model->get_circular(array('search'=>''));
	...
	function index()
	{
		...
		$data['meta']		= array(
			...
			$this->template->build('newsletter/newsletter', $data);
	...
	function add()
	{
		...
		else
		{
			...
			{
				$this->template->set_partial('navigation', 'dashboard_navigation');
				$this->template->build('newsletter/add', $data);
			}
	  	}
	}
	public function upload_files()
	{
		...
		else
		{
			...
			else
			{
				$data 										= $this->upload->data();
				$data_save  							= array();
				$data_save['n_title'] 		= $this->input->post('c_title');
				$data_save['n_slug'] 		  = str_replace(' ', '_',$this->input->post('c_title'));
				$data_save['n_filename'] 	= $data['file_name'];
				$data_save['n_file_path']	= base_url('uploads/newsletters/' . $data['file_name']);
				//echo "<pre>"; print_r($data);die;
				if($this->model->save_circular($data_save)	)
				{
				$array 	= array(
					'error'=>false,
					'filelink' => base_url('uploads/newsletters/' . $data['file_name']),
					'filename' => $data['file_name']
				);
