	...
	function getNewSnapshotHome($limit = 10)
	{
		...
		if($query->num_rows() > 0)
		{
			...
			foreach($query->result_array() as $c)
			{
				$posts .= '
					<div class="item' . ($s == 0 ? ' active' : '') . '">
						<div style="background:url(' . base_url('uploads/snapshots/' . imageCheck('snapshots', $c['snapshotFile'], 1)) . ') center center no-repeat;background-size:cover;-moz-background-size:cover;height:300px" class="rounded-sm"></div>
						<div class="carousel-caption">
							<h3 class="text-shadow"> &nbsp; </h3>
							<div class="clearfix"></div>
							<p class="text-shadow">' . ($c['snapshotContent'] != '' ? truncate($c['snapshotContent'], 80) : phrase('no_content')) . '</p>
